...
Subjects in TEST Group: **`r unique(tt)`**

Subjects in TEST Group: **`r unique(tr)`**
...
```{r Function for Aggregating Inertial Data}
...
```{r Aggregate Test Inertial Data}
...
```{r Aggregate Train Inertial Data}
for (stat in InertialStats) {
...
```{r Prepend Activity Determinations}
...
```{r Check Train and Test Table Metadata}
